...
public:
    explicit ISR() {
        m3::ISR::init(reinterpret_cast<uintptr_t>(&isr_state + 1));
    ...
    // we save our state here on IRQs; however, this is only used on RISC-V. x86 and arm just
    // continue with the current SP, because the privileged level is not changed.
    static m3::ISR::State isr_state;
    static ISR irqs;
};

m3::ISR::State ISR::isr_state;
ISR ISR::irqs;
