...
void *TCUState::get_ep(epid_t ep) {
    return _regs._eps + ep * m3::TCU::EPS_RCNT;
}

void TCUState::restore(const VPEDesc &) {
    // not supported
}

void TCUState::update_recv(epid_t ep, goff_t base) {
    word_t *regs = reinterpret_cast<word_t*>(get_ep(ep));
    regs[m3::TCU::EP_BUF_ADDR]       += base;
}

void TCUState::invalidate_ep(epid_t ep) {
    word_t *regs = reinterpret_cast<word_t*>(get_ep(ep));
    memset(regs, 0, sizeof(word_t) * m3::TCU::EPS_RCNT);
}

void TCUState::config_recv(epid_t ep, vpeid_t, goff_t buf, uint order, uint msgorder, uint) {
    word_t *regs = reinterpret_cast<word_t*>(get_ep(ep));
...
void TCUState::config_send(epid_t ep, vpeid_t, label_t lbl, peid_t pe, epid_t dstep, uint msgsize, uint credits) {
    word_t *regs = reinterpret_cast<word_t*>(get_ep(ep));
    regs[m3::TCU::EP_VALID]         = 1;
