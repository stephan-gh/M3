...
EthernetFrame TxFrame;	/* Frame used to send with */
EthernetFrame RxFrame;	/* Frame used to receive data */

volatile int Padding = 0;	/* For 1588 Packets we need to pad 8 bytes time stamp value */
volatile int ExternalLoopback = 0; /* Variable for External loopback */
...
void AxiEthernetUtilFrameSetVlanPayloadData(EthernetFrame *FramePtr,
					u32 PayloadSize, u32 VlanNumber)
{
	unsigned BytesLeft = PayloadSize;
	u8 *Frame;
	u16 Counter = 0;
...
/* Marvel PHY 88E151x Specific definitions */
//Ethernet FMC card has Marvel PHY 88E1510
#define PHY_R21_2_MAC_CTRL_REG   21
#define PHY_R20_18_GEN_CTRL_REG  20
...
#define MARVEL_PHY_88E151X_MODEL	0x1D0
								...
								int Speed)
{
	...
	u16 PhyReg21_2; // MAC Specific Control Register 2, page 2 of Marvell 88E151x PHY
	u16 PhyReg21_2_read;
	...
	if ((PhyType == XAE_PHY_TYPE_RGMII_2_0) ||
		...
		else if (PhyModel == MARVEL_PHY_88E151X_MODEL) {
			/*
			 * Switching to PAGE2
			 */
			XAxiEthernet_PhyWrite(AxiEthernetInstancePtr,
...
*
******************************************************************************/
void AxiEthernetUtilErrorTrap(char const *Message)
{
	//static int Count = 0;
	//Count++;
