...
impl<'a> TcpSocket<'a> {
    ...
    pub fn connect(&mut self, remote_addr: IpAddr, remote_port: u16, local_addr: IpAddr, local_port: u16) -> Result<(), Error>{
	self.socket.nm.connect(self.socket.sd, remote_addr, remote_port, local_addr, local_port)?;
	if self.blocking{
	...
	self.socket.nm.listen(self.socket.sd, local_addr, local_port)?;
	if self.blocking{
	...
	if self.blocking{
		...
		match self.socket.nm.recv(self.socket.sd){
		    ...
		    Err(e) => match e.code(){
			Code::NoSuchSocket | Code::SocketClosed | Code::InvState => return Err(Error::from(e)),
	...
	self.socket.nm.send(self.socket.sd, IpAddr::unspecified(), 0, IpAddr::unspecified(), 0, data)
    }
