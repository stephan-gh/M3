...
public:
    ...
    static const epid_t EP_RECV     = 9;
    static const epid_t EP_IN_SEND  = 10;
    static const epid_t EP_IN_MEM   = 11;
    static const epid_t EP_OUT_SEND = 12;
    static const epid_t EP_OUT_MEM  = 13;
    ...
    void connect_output(StreamAccel *next) {
        ...
        _mgate_out->activate_on(EP::bind_for(*_vpe, EP_OUT_MEM));
...
private:
    void connect_file(GenericFile *file, epid_t sep, epid_t mep, capsel_t scap) {
        file->sgate().activate_on(EP::bind_for(*_vpe, sep));
        file->sess().delegate_obj(EP::bind_for(*_vpe, mep).sel());
