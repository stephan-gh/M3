 ...
 */
struct __attribute__((aligned(4),packed)) NetData{
    ///Constructs a new NetData package from all context informations
    explicit NetData(int32_t sd, const uint8_t *data, uint32_t data_size, IpAddr src_addr, uint16_t src_port, IpAddr dst_addr, uint16_t dst_port){

	//Throw an error if the data size is too big
	if (data_size > MAX_NETDATA_SIZE){
	    LLOG(NET, "Packages size was too big when creating NetData. Max size=" << MAX_NETDATA_SIZE << ", package size=" << data_size);
     ...
     */
    void log(){
	LLOG(NET, "sd=" << sd << ", size=" << size << ", src_addr=" << src_addr.addr() << ", src_port=" << src_port << ", dst_addr=" << dst_addr.addr() << ", dst_port=" << dst_port << " data_as_string=" << (char*) data);
    }
