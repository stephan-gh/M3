...
//Thin wrapper around the NetElement so we dont destroy the NetData memory layout
	...
	RecvElement(m3::net::NetData* first_element)
	    : m3::TreapNode<RecvElement, int32_t>(first_element->sd),
	    waiting_packages(){
...
public:
    ...
    void connect(int32_t sd, IpAddr remote_addr, uint16_t remote_port, IpAddr local_addr, uint16_t local_port);
    void close(int32_t sd);
    void as_file(int32_t sd, int mode, MemGate &mem, size_t memsize, fd_t &fd);
    void notify_drop(int32_t sd);
    void send(int32_t sd, IpAddr src_sddr, uint16_t src_port, IpAddr dst_addr, uint16_t dst_port, uint8_t *data, uint32_t data_length);
