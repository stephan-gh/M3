...
fn simple_bandwidth(){
    ...
    wv_perf!(
		...
		for pkt_size in &packet_sizes{
			...
			let recved_time = u64::from_be_bytes([pkg.raw_data()[0], pkg.raw_data()[1], pkg.raw_data()[2], pkg.raw_data()[3], pkg.raw_data()[4], pkg.raw_data()[5], pkg.raw_data()[6], pkg.raw_data()[7]]);
