...
impl SendQueue {
    ...
    pub fn abort(&mut self) {
        log!(ROOT, "{}:squeue: aborting", self.serv_name());
    ...
    pub fn send(&mut self, msg: &[u8]) -> Result<thread::Event, Error> {
        log!(ROOT, "{}:squeue: trying to send msg", self.serv_name());

        if self.state == QState::Idle {
            return self.do_send(alloc_qid(), msg);
        }

        log!(ROOT, "{}:squeue: queuing msg", self.serv_name());
    ...
    fn received_reply(&mut self, rep: dtu::EpId, msg: &'static dtu::Message) {
        log!(ROOT, "{}:squeue: received reply", self.serv_name());
    ...
    fn send_pending(&mut self) {
        loop {
            match self.queue.pop_front() {
                ...
                Some(e) => {
                    log!(ROOT, "{}:squeue: found pending message", self.serv_name());
    ...
    fn do_send(&mut self, id: u64, msg: &[u8]) -> Result<thread::Event, Error> {
        log!(ROOT, "{}:squeue: sending msg", self.serv_name());
